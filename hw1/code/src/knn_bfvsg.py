...
bf_knn_file = os.path.join(data_path, "knn_results.csv")
...
plt.plot(bf_knn_df["k"], bf_knn_df["time(ms)"], marker='o', linestyle='-', label="Brute Force")
...
plt.plot(kd_knn_df["k"], kd_knn_df["time(ms)"], marker='o', linestyle='-', label="KD-Tree")
...
for file in grid_knn_files:
    ...
    if not grid_knn_df.empty:
        plt.plot(grid_knn_df["k"], grid_knn_df["time(ms)"], marker='o', linestyle='-', label=f"Grid (cell_size={cell_size})")

plt.xlabel("k")
plt.ylabel("Execution Time (ms)")
plt.title("kNN Execution Time Comparison (N=1630238)")
plt.legend()
plt.grid()
plt.savefig(f"../fig/knn_bfvsgrid.png")
plt.show()
